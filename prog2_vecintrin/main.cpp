...
float arraySumVector(float* values, int N) {
  ...
  //
  __cs149_vec_float sumVec = _cs149_vset_float(0.0f); // Vectorized sum
  ...
  for (int i = 0; i < N; i += VECTOR_WIDTH) {
      ...
      // Load values into a vector register
      __cs149_vec_float vec;
      _cs149_vload_float(vec, values + i, maskAll);

      // Sum vector elements
      _cs149_vadd_float(sumVec, sumVec, vec, maskAll);
  }

  // Horizontal sum to reduce vector to a single float
  float finalSum = 0.0f;
  float sumArray[VECTOR_WIDTH];
  _cs149_vstore_float(sumArray, sumVec, maskAll);
